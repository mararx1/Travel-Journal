...
- **Apply to site** (top bar, separate from Generate / disabled Publish) upserts `src/data/applied-stories.json` by slug, copies new derivatives into `public/images/stories/` via `POST /__studio/apply-to-site`, runs `npm run build`, and rolls back the write if the build fails. Blocking warnings (title/cover/missing file/slug collision) require explicit confirm. Never writes the hand-authored `chiatura-caves` content key. Pill advances Draft → Ready only after apply+build succeed (not “Published”).
